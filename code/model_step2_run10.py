...
def model_a():
    ...
    node_range = np.asarray([32, 64, 128, 256])
    ...
    for i in range(30):
        ...
        c1_filters = np.random.choice(node_range, 1)[0]
        c2_filters = np.random.choice(node_range, 1)[0]
        c3_filters = np.random.choice(node_range, 1)[0]
        d1_units = np.random.choice(node_range, 1)[0]
        d2_units = np.random.choice(node_range, 1)[0]
        ...
        custom_results.append([model_id, history.epoch[-1], elapsed,
                               c1_filters, c2_filters, c3_filters,
                               d1_units, d2_units, acc, val_acc])
    cols = ['model', 'epochs', 'time', 'conv1', 'conv2', 'conv3', 'dense1',
            'dense2', 'acc', 'val_acc']
